...
def calculate_slope_intercept(x1, y1, x2, y2):
    ...
    # Calculate the slope (m) and y-intercept (c) of the line passing through the two points
    m = (y2 - y1) / (x2 - x1)
    c = y1 - m * x1
    
    return m, c

def is_point_on_line(x, y, line_slope, line_y_intercept):
    m = line_slope
    c = line_y_intercept
...
# Function to get current process CPU and memory usage
    ...
    for i in range(0, len(lines)):
        for x1, y1, x2, y2 in lines[i]:
            ...
            if (((diff1<diff1UpperBound*r) and (diff1>diff1LowerBound*r) and (diff2<diff2UpperBound*r)) and (diff2>diff2LowerBound*r)):
                ...
                line_slope, line_y_intercept = calculate_slope_intercept(x1, y1, x2, y2)
                line_along_pt_flag = is_point_on_line(x, y, line_slope, line_y_intercept)
                # add to final list
